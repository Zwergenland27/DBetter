...
{
    private static TrainInformation Create(string fullName)
    {
        ...
        return new TrainInformation(product, trainLine, trainNumber);
    }

    public static List<TrainInformation> Create(string leadingVehicleName, string fullName)
    {
        var fullNames = fullName.Split(" / ")
            .OrderBy(n => !n.Contains(leadingVehicleName))
            .ToList();
        return fullNames.Select(TrainInformationFactory.Create).ToList();
    }
    ...
    private static TrainLine? GetTrainLine(string? trainLine)
    {
        if (trainLine is null) return null;
        return new TrainLine(trainLine);
    }

    public static TrainNumber? GetTrainNumber(string? trainNumber)
    {
